...
/// Polynomials over the ring Z (defined in intMod).
struct Poly: CustomStringConvertible {
    ...
    init (_ coeffDict: [Int:Z]) {
        self.coeffs = Array.init(repeating: Z.addId(), count: Self.exponentModulus)
    ...
    static func multId() -> Self { return Poly([0:Z.multId()]) }
    ...
    private static var sizedEmptyArray: [Z] {
        return Array.init(repeating: Z.addId(), count: Self.exponentModulus)
    }
    ...
    var description: String {
        ...
        for i in Self.expRange {
            guard self[i] != Z.addId() else { continue }
            outStr += "\(self[i])(\(i)), "
        }
        return outStr
    }
    ...
    var negated: Poly {
        return self.scale(by: Z.multId().negated)
    }
    
    /// Scalar multiplication.
    func scale(by n: Z) -> Poly {
        var newPoly = self
    ...
    /// Polynomial squared.
    var squared: Poly {
        var new: [Z] = Self.sizedEmptyArray
        for i in Self.expRange {
