...
extension Array where Element == R {
  
  /// Add a c-multiple of arr onto the current array.
  mutating func addMultiple(of arr: [R], scalar c: R) {
...
struct R: Equatable, CustomStringConvertible {
  ...
  /// Additive identity.
  static func addId() -> Self { R(0) }
  
  /// Multiplicative identity.
  static func multId() -> Self { R(1) }
  ...
  var negated: R {
    return R(Self.modulus - self.repr)
  }
